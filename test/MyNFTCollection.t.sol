...
contract MyNFTCollectionTest is Test {
    ...
    function test_SuccessfulMint() public {
        ...
        assertEq(
            address(nft).balance,
            nft.MINT_PRICE() * mintAmount,
            "Contract balance mismatch"
        );
    }

    function test_RevertWhen_MintMoreThanLimitPerTx() public {
        vm.deal(user1, TEST_FUNDS);
        vm.prank(user1);
    ...
    function test_RevertWhen_ExceedsWalletLimit() public {
        ...
        vm.expectRevert(MyNFTCollection.ExceedsWalletLimit.selector);
        nft.mint{value: nft.MINT_PRICE()}(1, user1);
        vm.stopPrank();
    }

    function test_RevertWhen_MintUnderpayment() public {
        vm.deal(user1, TEST_FUNDS);
        vm.prank(user1);
        vm.expectRevert(MyNFTCollection.IncorrectETHValue.selector);
    ...
    function test_MintToDifferentRecipient() public {
        vm.deal(user1, TEST_FUNDS);
        vm.prank(user1);
        nft.mint{value: nft.MINT_PRICE() * nft.MAX_PER_TX()}(
            nft.MAX_PER_TX(),
            user2
        );
    ...
    function test_BalanceOfAfterMultipleMints() public {
        vm.deal(user1, TEST_FUNDS);
        vm.prank(user1);
        nft.mint{value: nft.MINT_PRICE() * (nft.MAX_PER_TX() - 1)}(
            nft.MAX_PER_TX() - 1,
            user1
        );
        ...
        assertEq(
            nft.balanceOf(user1),
            nft.MAX_PER_TX(),
            "Accumulated balance mismatch"
        );
    }

    function test_RevertWhen_WithdrawAsNotOwner() public {
    ...
    function test_WithdrawAsOwner() public {
        ...
        assertEq(
            address(nft).balance,
            contractBalance,
            "Contract should have correct balance"
        );
